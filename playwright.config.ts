...
export default defineConfig({
  ...
  projects: [
    ...
    // Legacy directory `tests/e2e/` (audit §5.1, §6.2.8 — previously
    // invisible to the default Playwright invocation). Predates `e2e/`
    // and uses direct-login boilerplate; all specs share the seeded
    // admin org so they must run serially. Top-level `globalSetup` /
    // `globalTeardown` apply; only `testDir` is overridden per project.
    {
      name: "tests-e2e",
      testDir: "./tests/e2e",
      testMatch: /.*\.spec\.ts$/,
