 ...
 * hard-coded a plaintext password for a specific admin account and force-reset
 * that account's password + role and the org's plan tier whenever they had
 * drifted. That embedded a working credential in committed git history and
 * silently reverted any password rotation / role change on the next deploy.
 *
 * The fix removed the block. The contract was verified BEHAVIORALLY during review
 ...
 * persistent guard pins the invariant STRUCTURALLY instead, so it is
 * deterministic and does not run the full boot migration (which performs global
 * DDL on the shared test DB) inside the parallel suite: a boot data-migration
 * must never embed a known credential or reset an existing account's password.
 */
...
describe("boot migration never embeds or resets a user's credentials (audit #11)", () => {
  it("performs no boot-time password reset (value-agnostic)", () => {
