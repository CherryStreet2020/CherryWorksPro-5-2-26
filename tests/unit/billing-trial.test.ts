...
import { checkoutTrialFor, STRIPE_MIN_TRIAL_MS, TRIAL_SAFETY_MARGIN_MS } from "../../server/billing-trial";
...
describe("checkout trial policy", () => {
  it("keeps the signup deadline mid-trial, never restarts it", () => {
    ...
    expect(t).toEqual({ trial_end: Math.ceil((now.getTime() + 9 * DAY) / 1000) });
  });
  it("extends only to Stripe's 48-hour minimum when less than that is left", () => {
    ...
    expect(t).toEqual({ trial_end: Math.ceil((now.getTime() + STRIPE_MIN_TRIAL_MS + TRIAL_SAFETY_MARGIN_MS) / 1000) });
    // Still ≥ 48h when Stripe validates a few minutes later.
    expect(t!.trial_end * 1000 - (now.getTime() + 5 * 60 * 1000)).toBeGreaterThanOrEqual(STRIPE_MIN_TRIAL_MS);
  });
