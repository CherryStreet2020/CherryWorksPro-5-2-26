...
| 15 | Marketing OS: env flag `MARKETING_OS_ENABLED` off ⇒ all `/marketing/*` 404 even if entitlement is present, and the marketing sidebar section is hidden | Covered end-to-end by paired specs `e2e/feature-flag-marketing-os.flags-{on,off}.spec.ts` running against two dev servers (the dev workflow + a `playwright.feature-flags-off.config.ts`-spawned server with all four flag env vars set to false). Kill switch is wired into the `requireFeature("marketing_os")` chokepoint middleware on the server side, and `client/src/lib/featureFlags.ts` now reads `import.meta.env.VITE_MARKETING_OS_ENABLED` so the sidebar gate flips with the env. EMAIL_OAUTH coverage in `e2e/feature-flag-email-oauth.flags-{on,off}.spec.ts` includes oauth-start/callback 404, settings UI showing SMTP-only when OFF, and an SMTP-path smoke (Task #437). |
