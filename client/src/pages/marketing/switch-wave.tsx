...
const painPoints = [
  { pain: "Wave has no time tracking at all", fix: "CherryWorks Pro has native time tracking with week, month, and day views, a floating timer widget, and full timesheet approval workflows." },
  { pain: "No project management or profitability", fix: "Track project budgets, labor cost vs. bill rates, and see profitability per project with built-in reports." },
...
const comparison = [
  { feature: "Unlimited users (no per-seat fees)", cw: true, comp: true },
  { feature: "Native time tracking", cw: true, comp: false },
  { feature: "Timesheet approval workflow", cw: true, comp: false },
  ...
  { feature: "Project profitability (labor + expenses)", cw: true, comp: false },
  { feature: "Client portal with overdue alerts", cw: true, comp: false },
  { feature: "Expense management with approvals", cw: true, comp: false },
  { feature: "Import wizard (upload, preview, rollback)", cw: true, comp: false },
  { feature: "Basic invoicing", cw: true, comp: true },
  { feature: "Online payments", cw: true, comp: true },
  { feature: "General ledger accounting", cw: true, comp: true },
  { feature: "Bank reconciliation", cw: true, comp: true },
...
export default function SwitchWavePage() {
    ...
    <div style={{ background: "#0a0f1c" }}>
      ...
      <section className="pt-[100px] pb-8 md:pb-10" style={{ background: "var(--gradient-hero)" }}>
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-8 md:pt-12 md:pb-10">
          <div className="max-w-3xl">
            <p className="text-sm font-bold uppercase tracking-[3px] mb-4" style={{ color: "#cf3339" }}>Switch from Wave</p>
            <h1 className="text-4xl md:text-6xl font-bold text-white tracking-tight leading-[1.1]">
              Wave was free.{" "}
              <span style={{ color: "rgba(255,255,255,0.4)" }}>Now you need more.</span>
            </h1>
            <p className="mt-6 text-lg md:text-xl leading-relaxed" style={{ color: "rgba(255,255,255,0.55)" }}>
              Your firm outgrew basic invoicing. You need time tracking, project profitability, team payouts, expense approvals, and 1099 exports. CherryWorks Pro has everything you're missing.
            </p>
      ...
      <section className="py-8 md:py-12" style={{ background: "#0a0f1c" }}>
        <div ref={painRef} className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 fade-in-section">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-4xl font-bold text-white">Why firms switch from Wave</h2>
            <p className="mt-4 text-lg" style={{ color: "rgba(255,255,255,0.45)" }}>Every one of these is something Wave can't do — or doesn't offer at all.</p>
          </div>
      ...
      <section className="py-8 md:py-12" style={{ background: "linear-gradient(180deg, #0f172a 0%, #0a0f1c 100%)" }}>
        <div ref={compRef} className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 fade-in-section">
          <div className="text-center mb-12">
            ...
            <p className="mt-3 text-base" style={{ color: "rgba(255,255,255,0.45)" }}>Where CherryWorks Pro wins — and what Wave still offers.</p>
          </div>
          <div className="rounded-2xl overflow-hidden" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="grid grid-cols-3 text-center py-4 px-6" style={{ background: "rgba(255,255,255,0.04)" }}>
              ...
              <div><span className="text-sm font-bold" style={{ color: "rgba(255,255,255,0.5)" }}>Wave</span></div>
            </div>
            {comparison.map((r, i) => (
          ...
          <p className="mt-6 text-sm text-center" style={{ color: "rgba(255,255,255,0.35)" }}>
            Wave is great for basic bookkeeping. CherryWorks Pro is built for professional services operations.
          </p>
      ...
      <section className="py-8 md:py-12" style={{ background: "#0a0f1c" }}>
        <div ref={timelineRef} className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 fade-in-section">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: Upload, step: "1", title: "Export from Wave", desc: "Export your clients, invoices, and accounting data as CSV files from Wave." },
      ...
      <section className="py-8 md:py-12" style={{ background: "linear-gradient(180deg, #0f172a 0%, #0a0f1c 100%)" }}>
        <div ref={getRef} className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 fade-in-section">
          <div className="text-center mb-14">
            <h2 className="text-3xl md:text-4xl font-bold text-white" data-testid="what-you-get-heading">What you've been missing</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {[
              { icon: Clock, title: "Native time tracking", desc: "Week, month, and day views with a floating timer widget. Wave doesn't offer this at all." },
              { icon: Shield, title: "Expense approvals", desc: "Submit, approve, reimburse — with receipt uploads and full audit trail." },
              { icon: Zap, title: "Unlimited users", desc: "No per-seat fees, ever. Add your entire team on any plan." },
              { icon: Upload, title: "Project profitability", desc: "Revenue minus labor cost minus expenses per project. See exactly where you're making money." },
            ].map((b, i) => (
      ...
      <section className="py-10 md:py-14" style={{ background: "#0a0f1c" }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Ready to switch from Wave?</h2>
          <p className="text-lg mb-10" style={{ color: "rgba(255,255,255,0.5)" }}>
            14-day free trial. Full access. Import your Wave data in minutes.
          </p>
          <Link href="/signup">
